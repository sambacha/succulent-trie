...
switch (argv[2]) {
	case "optimized":
		Tree = require("ztrie/optimized").Tree;
		argv.splice(2, 1);
		break;
	case "trie":
		Tree = require("ztrie/trie").Tree;
		argv.splice(2, 1);
		break;
	case "radix":
		Tree = require("ztrie/radix").Tree;
		argv.splice(2, 1);
		break;
	default:
		Tree = require("ztrie/optimized").Tree;
...
async function stress(prefix) {
	...
	console.log("Committing %d values to tree at a rate of %d per block.", TOTAL, PER_BLOCK);
...
async function doProof(tree, i, key, expect) {
	...
	if (proof.nodes.length > 0) {
		if (Buffer.isBuffer(proof.nodes[0])) {
			...
			for (const { prefix, node } of proof.nodes) size += 2 + prefix.data.length + node.length;
		}
	}
	...
	console.log("Proof %d compressed size: %d", i, proof.getSize(tree.hash, tree.bits));
}
