...
async function main() {
	...
	for (let j = 0; j < 200; j++) {
		...
		key = k;
		const { root, proof } = await Cztrie.put("new_index", k, v);
	}
	...
	//const ar = await Cztrie.range("new_index", 0, 50)
	var b = randomBytes(100);
	var n = 10;
	n = unpack(n.toString(), 16);

	const arr = await Cztrie.filter("new_index", (key, value) => {
...
async function main1() {
	// Create a tree using blake2b-256 and a depth/key-size of 256 bits.
	const tree = await Cztrie.setup("new_index");

	const iter = tree.iterator();

	while (await iter.next()) {
		const { key, v } = await iter;
		const { value, proof } = await Cztrie.get(tree, k);
