...
const optimize = require("ztrie/optimized");
const randomBytes = require("randombytes");
const { SHA256 } = bcrypto;
const { Tree, Proof } = optimize;
const CztrieClient = require("../proto/client");
const { promisify } = require("util");
const put = promisify(CztrieClient.put);
...
async function main() {
	...
	for (let j = 0; j < 1; j++) {
		for (let i = 0; i < 1; i++) {
			...
			const resp = await CztrieClient.put(putRequest);
...
function putCalls(putR, n) {
	...
	CztrieClient.put(putR, (error, response) => {
		if (!error) {
...
function getCalls(getR, n) {
	...
	CztrieClient.get(getR, (error, response) => {
		if (!error) {
