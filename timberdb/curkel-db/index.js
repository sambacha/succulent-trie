...
async function range(indexName, start, finish) {
	const index = await setup(indexName);
	const values = await Cztrie.range(index, start, finish);
	await teardown(indexName, index);
	return values;
}
...
async function filter(indexName, pred) {
	const index = await setup(indexName);
	const values = await Cztrie.filter(index, pred);
	await teardown(indexName, index);
	return values;
}
...
async function put(indexName, key, value) {
	const index = await setup(indexName);
	const txn = await Cztrie.checkout(index);
	const { root, p } = await Cztrie.put(index, txn, key, value);
	await teardown(indexName, index);
	return { root, p };
}
...
async function getSync(indexName) {
	const index = await setup(indexName);
	const values = await Cztrie.synchronize(index, pred);
	await teardown(indexName, index);
	return values;
}
...
async function synchronize(indexName, values) {
	const index = await setup(indexName);
	const txn = await Cztrie.checkout(index);
	const root = await Cztrie.synchronize(index, txn, values);
	await teardown(indexName, index);
	return root;
}
...
async function del(indexName, key) {
	const index = await setup(indexName);
	const txn = await Cztrie.checkout(index);
	const response = await Cztrie.del(index, key);
	await teardown(indexName, index);
	return response;
}
