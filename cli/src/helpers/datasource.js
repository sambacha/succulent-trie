...
function processEvent(contractEvent) {
    return {
        schema: eventSchema(contractEvent),
        schemaHead: eventQueries(contractEvent),
        eventStruct: eventResolvers(contractEvent),
    ...
    for (let v = 0; v < contractEvent.inputs.length; v++) {
        switch (contractEvent.inputs[v].type) {
            case "address":
                funct +=
                    "   " +
                    contractEvent.inputs[v].name +
                    ": " +
                    "contractEvent.value.returnValues." +
                    contractEvent.inputs[v].name +
                    ",\n";
                break;
            case "uint256":
                funct +=
                    ...
                    contractEvent.inputs[v].name +
                    ": " +
                    "web3.utils.fromWei(contractEvent.value.returnValues." +
                    contractEvent.inputs[v].name +
                    "._hex" +
                    ", 'ether')" +
                    ",\n";
